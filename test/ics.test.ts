...
test("createEvent builds a valid event with exclusive all-day end", () => {
  const ev = createEvent({ summary: "Test", allDay: true, start: "2026-07-03", end: "2026-07-03", tzid: null })
...
test("createEvent rejects end not after start", () => {
  expect(createEvent({ summary: "X", allDay: false, start: "2026-07-03 10:00", end: "2026-07-03 09:00", tzid: null })).toBeNull()
})
...
test("applyEdit updates fields and increments sequence", () => {
  ...
  const ok = applyEdit(ev, { summary: "Revisão carro v2", allDay: false, start: "2026-01-19 11:00", end: "2026-01-19 12:00", tzid: getTzid(ev) })
  expect(ok).toBe(true)
...
test("applyEdit can switch timed to all-day", () => {
  const cal = parseCalendar(SAMPLE)
  const ev = cal.events[1]!
  const ok = applyEdit(ev, { summary: "AllDay", allDay: true, start: "2026-01-19", end: "2026-01-19", tzid: null })
  expect(ok).toBe(true)
