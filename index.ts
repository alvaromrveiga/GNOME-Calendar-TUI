import { createCliRenderer, BoxRenderable, TextRenderable, InputRenderable, t, fg, bg, bold, underline, type KeyEvent, type CliRenderer } from "@opentui/core"
import {
  eventToForm,
  createEvent,
  applyEdit,
...
let focusIndex = 0
let form: { box: BoxRenderable; summary: InputRenderable; allDay: InputRenderable; start: InputRenderable; end: InputRenderable } | null = null
...
interface Args { dir: string | null, file: string | null }
...
function parseArgs(): Args {
  ...
  while (i < argv.length) {
    const a = argv[i]
    if (a === "--dir" && argv[i + 1]) { out.dir = argv[i + 1]!; i += 2; continue }
    if (a?.startsWith("--dir=")) { out.dir = a.slice(6); i++; continue }
    if (a === "--file" && argv[i + 1]) { out.file = argv[i + 1]!; i += 2; continue }
    if (a?.startsWith("--file=")) { out.file = a.slice(7); i++; continue }
    i++
  }
...
function renderStatus(): void {
  ...
  if (statusMsg) {
    ...
    left = t` ${fg(ACCENT)("NORMAL")}  ${fg(DIM)("j/k move · e/Enter edit · n new · d delete · r reload · ? help · q quit")}`
  } else if (mode === "edit") {
...
function renderList(): void {
  ...
  if (entries.length === 0) {
    listBox.add(new TextRenderable(renderer, { content: " No events. Press 'n' to create one.", fg: DIM }))
    return
  }
  const vr = visibleRows()
...
function labeledInput(label: string, input: InputRenderable): BoxRenderable {
  const row = new BoxRenderable(renderer, { flexDirection: "row", gap: 1, alignItems: "center" })
...
function openEdit(isNew: boolean): void {
  ...
  const summary = new InputRenderable(renderer, { id: "f-summary", value: data.summary, placeholder: "Event title", width: 40, backgroundColor: "#161b22", focusedBackgroundColor: "#1f2937", textColor: TEXT, cursorColor: ACCENT })
  const allDay = new InputRenderable(renderer, { id: "f-allday", value: data.allDay ? "y" : "n", placeholder: "y/n", width: 40, backgroundColor: "#161b22", focusedBackgroundColor: "#1f2937", textColor: TEXT, cursorColor: ACCENT })
  const start = new InputRenderable(renderer, { id: "f-start", value: data.start, placeholder: "YYYY-MM-DD HH:MM", width: 40, backgroundColor: "#161b22", focusedBackgroundColor: "#1f2937", textColor: TEXT, cursorColor: ACCENT })
  const end = new InputRenderable(renderer, { id: "f-end", value: data.end, placeholder: "YYYY-MM-DD HH:MM", width: 40, backgroundColor: "#161b22", focusedBackgroundColor: "#1f2937", textColor: TEXT, cursorColor: ACCENT })
  ...
  box.add(new TextRenderable(renderer, { content: " Start/End: YYYY-MM-DD  or  YYYY-MM-DD HH:MM", fg: DIM }))
  box.add(new TextRenderable(renderer, { content: " [Enter] save   [Tab]/[S-Tab] next/prev   [Esc] cancel", fg: DIM }))
...
function openConfirmDelete(): void {
  ...
  box.add(new TextRenderable(renderer, { content: t` ${fg(ERROR)("Delete")} "${sum}"?`, fg: TEXT }))
  if (multiCalendar()) {
    box.add(new TextRenderable(renderer, { content: ` From: ${entry.calendarName}`, fg: DIM }))
  }
...
function openHelp(): void {
  ...
  for (const l of lines) {
    box.add(new TextRenderable(renderer, { content: l, fg: l.startsWith(" ") && l.trim().length > 0 ? TEXT : DIM }))
  }
  overlayBox.add(box)
  overlayBox.visible = true
  helpBox = box
...
async function main(): Promise<void> {
  ...
  renderer = await createCliRenderer({ exitOnCtrlC: false, backgroundColor: BG, useMouse: false })

  appBox = new BoxRenderable(renderer, { id: "app", width: "100%", height: "100%", flexDirection: "column", backgroundColor: BG })
  titleText = new TextRenderable(renderer, { id: "title", content: " Fedora Calendar" })
  colHeaderText = new TextRenderable(renderer, { id: "colhead", content: " Date       Time    Summary" })
  listBox = new BoxRenderable(renderer, { id: "list", flexGrow: 1, flexDirection: "column", backgroundColor: BG })
  statusText = new TextRenderable(renderer, { id: "status", content: "", fg: DIM, width: "100%" })
  overlayBox = new BoxRenderable(renderer, { id: "overlay", position: "absolute", top: 0, left: 0, width: "100%", height: "100%", flexDirection: "row", justifyContent: "center", alignItems: "center", backgroundColor: BG, visible: false })
